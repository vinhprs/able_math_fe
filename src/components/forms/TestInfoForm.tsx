...
const testInfoSchema = z.object({
  ...
  term: z.nativeEnum(Term, { required_error: 'Term is required' }),
  level: z.enum(['1', '2', '3'], { required_error: 'Level is required' }).transform(Number),
...
export function TestInfoForm({ initialData, onSubmit, generatedTestCode }: TestInfoFormProps) {
  const {
    register,
    handleSubmit,
    watch,
    formState: { errors },
  } = useForm<TestInfoFormData>({
    resolver: zodResolver(testInfoSchema),
    defaultValues: initialData
      ? {
          ...
          level: initialData.level?.toString() as '1' | '2' | '3' | undefined,
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      {generatedTestCode && (
        <div className="p-4 bg-primary-50 border border-primary-200 rounded-lg">
          <p className="text-sm font-medium text-primary-900 mb-1">Generated Test Code</p>
          <p className="text-lg font-mono font-semibold text-primary-700">{generatedTestCode}</p>
        </div>
      )}

      {watchedValues[0] && watchedValues[1] && watchedValues[2] && !generatedTestCode && (
        <div className="p-4 bg-secondary-50 border border-secondary-200 rounded-lg">
          <p className="text-sm text-secondary-600">
            Test code will be generated automatically after you fill in all fields.
          </p>
        </div>
      )}
