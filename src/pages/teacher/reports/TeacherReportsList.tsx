...
export function TeacherReportsList() {
  ...
  const loadReports = async () => {
    try {
      setLoading(true);
      console.log(
        "[TeacherReportsList] Loading reports with filters:",
        filters
      );
      const data = await reportService.getAllReportsForTeacher(filters);
      console.log(
        "[TeacherReportsList] Reports loaded:",
        data?.length || 0,
        "reports"
      );
      setReports(data || []);
    } catch (error: any) {
      console.error("[TeacherReportsList] Failed to load reports:", error);
  ...
  useEffect(() => {
    console.log(
      "[TeacherReportsList] Component mounted/filters changed, calling loadReports"
    );
    loadReports();
  ...
  const handlePublish = async (reportId: string) => {
    if (
      !confirm(
        "Publish this report? Student will be able to view it after publishing."
      )
    ) {
      return;
    }
  ...
  const handleViewReport = (report: TeacherReport) => {
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">
          Review, approve, and publish student reports
        </p>
      </div>
